...
public class ApplicationIntegrationTests : TestBase
{
    ...
    )
    {
        ...
        {
            new (UnitType.Peasant, "Test Peasant", 1, 1, 1, 1, 1, 1, 1, 1, 1, UnitCategory.Infantry, 1),
            new (UnitType.Invulnerable, "Test Invulnerable", 1, 1, 1, 1, 1, 1, 1, 1, 1,UnitCategory.Infantry,1)
        };
    ...
    public async Task Mediator_CancelTrain_ShouldHandleRulesCorrectly(bool isTrained, bool shouldSucceed)
    {
        ...
        var unitTemplates = new UnitTemplate[]
        {
            new(
                ...
                TurnsToRecruit: isTrained ? 0 : 1,UnitCategory.Infantry, 1, RequiredBuilding: BuildingType.ReqruitBarrack
            )
        };
    ...
    )
    {
        ...
        {
            new(
                UnitType.Peasant, "Test Unit", templateCost, 1, 1, 1, 1, 1, 1, 1, 1,UnitCategory.Infantry, 1,
    ...
    public async Task AiDiplomacy_ShouldMakeCorrectDecisions_BasedOnUtilityMath(
        ...
        OfferStatus expectedOfferStatus,
    ...
    )
    {
        ...
        [
            new(UnitType.Peasant, "TU", 0, MaxHealth: 100, Damage: 10, Armor: 10, 0, 0, 0, 0, 0,UnitCategory.Infantry, 1)
        ];
    ...
    private static int CalculateExpectedStanding(
        ...
        bool declareWar)
    {
        var current = alreadyTraded
            ? int.Clamp(initial + GameBalance.Diplomacy.AcceptTradeOfferBonus,
                        DiplomacyRelation.MinStanding, DiplomacyRelation.MaxStanding)
            : initial;

        if (offerType == OfferType.TradeAgreement)
        {
        ...
        if (declareWar)
        {
            current += GameBalance.Diplomacy.DeclareWarPenalty;
        }

        return int.Clamp(current, DiplomacyRelation.MinStanding, DiplomacyRelation.MaxStanding);
    }
